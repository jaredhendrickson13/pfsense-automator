...
def convert_xml(xml):
    # Local variables
    xmlDict = {}    # Init our return dictionary
    ...
    if type(xml) is str:
        ...
        try:
            ...
            pass    # Pass this exception for now, replace with more specific exception selector
...
def validate_platform(url,req_obj):
    # Local variables
    htmlStr = req_obj["text"] if req_obj is not None else http_request(url, {}, {}, {}, 45, "GET")["text"]    # Get our HTML data
...
def check_dns_rebind_error(url, req_obj):
    # Local Variables
    httpResponse = req_obj["text"] if req_obj is not None else http_request(url, {}, {}, {}, 45, "GET")["text"]    # Get the HTTP response of the URL
...
def check_errors(server, user, key, privList):
    # Local variables
    ec = 2    # Init our error code to 2
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    getBase = http_request(url, None, None, None, 45, "GET")    # Get our base URL to check for errors
    # Submit our intitial request and check for errors
    ec = 10 if check_dns_rebind_error(url, getBase) else ec    # Return exit code 10 if dns rebind error found
    ec = 6 if not validate_platform(url, getBase) else ec    # Check that our URL appears to be pfSense
    ...
    if ec == 2:
        # Check that we had permissions for each page in privList, if we do not, break and return error 15
        for p in privList:
            privCheck = http_request(url + "/" + p, None, None, None, 45, "GET")
            if not check_permissions(privCheck):
                ec = 15
                break
...
def get_pfsense_config(server, user, key, localXml):
    # Local variables
    pfConfig = {"ec": 2, "config": {}}    # Init our return dict
    xml = ""    # Init our XML string as blank
    localConfigXmlPath = "/conf/config.xml"    # Save the file path of our local config.xml file
    # Check if we are parsing a local file or a remote file
    if localXml == True:
        xml = localConfigXmlPath
    else:
        getRemoteConfigXml = get_shell_output(server, user, key, "cat " + localConfigXmlPath)    # Pull our XML config through webConfigurator
        # Check if we pulled our config correctly
        if getRemoteConfigXml["ec"] == 0:
            xml = getRemoteConfigXml["shell_output"]
        # If we did not pull our XML config successfully, return the exit code returned by get_shell_output()
        else:
            pfConfig["ec"] = getRemoteConfigXml["shell_output"]
    # Convert our XML to dict type and return success exit code
    if xml != "":
        pfConfig["config"] = convert_xml(xml)
        pfConfig["ec"] = 0
    # Return our dictionary
    return pfConfig
...
def get_pfsense_version(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    # Submit our intitial request and check for errors
    pfVersion["ec"] = 10 if check_dns_rebind_error(url, None) else pfVersion["ec"]    # Return exit code 10 if dns rebind error found
    pfVersion["ec"] = 6 if not validate_platform(url, None) else pfVersion["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if pfVersion["ec"] == 2:
...
def get_ha_sync(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    # Submit our intitial request and check for errors
    haSync["ec"] = 10 if check_dns_rebind_error(url, None) else haSync["ec"]    # Return exit code 10 if dns rebind error found
    haSync["ec"] = 6 if not validate_platform(url, None) else haSync["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if haSync["ec"] == 2:
...
def get_system_advanced_admin(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    # Submit our intitial request and check for errors
    advAdm["ec"] = 10 if check_dns_rebind_error(url, None) else advAdm["ec"]    # Return exit code 10 if dns rebind error found
    advAdm["ec"] = 6 if not validate_platform(url, None) else advAdm["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if advAdm["ec"] == 2:
...
def get_shell_output(server, user, key, cmd):
    ...
    shellOut["ec"] = 10 if check_dns_rebind_error(url, None) else shellOut["ec"]    # Return exit code 10 if dns rebind error found
    shellOut["ec"] = 6 if not validate_platform(url, None) else shellOut["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if shellOut["ec"] == 2:
    ...
    if shellOut["ec"] == 2:
        ...
        if check_permissions(getShellData):
            ...
            if "<pre>" in shellCmdPost["text"]:
                shellOut["shell_output"] = html.unescape(shellCmdPost["text"].split("<pre>")[1].split("</pre>")[0])    # Update our shell output value
...
def get_arp_table(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    # Submit our initial request and check for errors
    arpTable["ec"] = 10 if check_dns_rebind_error(url, None) else arpTable["ec"]    # Return exit code 10 if dns rebind error found
    arpTable["ec"] = 6 if not validate_platform(url, None) else arpTable["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if arpTable["ec"] == 2:
...
def get_xml_backup(server, user, key, area, noPkg, noRrd, encrypt, encryptPass):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    # Submit our intitial request and check for errors
    xmlTable["ec"] = 10 if check_dns_rebind_error(url, None) else xmlTable["ec"]    # Return exit code 10 if dns rebind error found
    xmlTable["ec"] = 6 if not validate_platform(url, None) else xmlTable["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if xmlTable["ec"] == 2:
...
def upload_xml_backup(server, user, key, area, confFile, decryptPass):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)  # Assign our base URL
    # Submit our intitial request and check for errors
    xmlAdded = 10 if check_dns_rebind_error(url, None) else xmlAdded    # Return exit code 10 if dns rebind error found
    xmlAdded = 6 if not validate_platform(url, None) else xmlAdded    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if xmlAdded == 2:
...
def replicate_xml(server, user, key, area, targetList):
    ...
    masterConfig = get_xml_backup(server, user, key, "", False, False, True, PfaVar.current_date)    # Get our XML configuration and save it to a variable
    ...
    if masterConfig["ec"] == 0:
        ...
        for tg in targetList:
            ...
            targetUpload = upload_xml_backup(tg, user, key, area, masterConfigBinary, PfaVar.current_date)    # Run our function and capture the exit code
...
def get_interfaces(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Assign our base URL
    ifacesXml = pfsensexml.get_interfaces_xml(server, user, key)    # Try to pull our interfaces through XML (for speed)
    # Check if we can read our interfaces via XML, if not, try via webconfigurator
    if ifacesXml["ec"] != 0:
        errCheck = check_errors(server, user, key, ["interfaces_assign.php", "interfaces.php"])    # Check for webconfigurator errors
                    ...
                    for pfId in pfIfList:
                        ...
                        for chk in toggleInputs:
                            ...
                            if "name=\"" + chk + "\"" in getIfConfig:
                                ifaces["ifaces"][pfId][chk] = "yes" if "checked=\"checked\"" in getIfConfig.split("name=\"" + chk + "\"")[1].split("</label>")[0] else ""
...
def add_auth_server_ldap(server, user, key, descrName, ldapServer, ldapPort, transport, ldapProtocol, timeout, searchScope, baseDN, authContainers, extQuery, query, bindAnon, bindDN, bindPw, ldapTemplate, userAttr, groupAttr, memberAttr, rfc2307, groupObject, encode, userAlt):
    ...
    ldapAdded = 10 if check_dns_rebind_error(url, None) else ldapAdded    # Return exit code 10 if dns rebind error found
    ldapAdded = 6 if not validate_platform(url, None) else ldapAdded    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if ldapAdded == 2:
...
def get_dns_entries(server, user, key):
    ...
    # Submit our intitial request and check for errors
    dnsDict["ec"] = 10 if check_dns_rebind_error(url, None) else dnsDict["ec"]    # Return exit code 10 if dns rebind error found
    dnsDict["ec"] = 6 if not validate_platform(url, None) else dnsDict["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if dnsDict["ec"] == 2:
...
def add_dns_entry(server, user, key, host, domain, ip, descr):
    ...
    if not check_dns(server, user, key, host, domain):
        # Check for errors and assign exit codes accordingly
        recordAdded = 10 if check_dns_rebind_error(url, None) else recordAdded    # Return exit code 10 if dns rebind error found
        recordAdded = 6 if not validate_platform(url, None) else recordAdded    # Check that our URL appears to be pfSense
        # Check if we have not encountered an error that would prevent us from authenticating
        if recordAdded == 2:
...
def get_ssl_certs(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Populate our base URL
    # Submit our intitial request and check for errors
    certManagerDict["ec"] = 10 if check_dns_rebind_error(url, None) else certManagerDict["ec"]    # Return exit code 10 if dns rebind error found
    certManagerDict["ec"] = 6 if not validate_platform(url, None) else certManagerDict["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
...
def add_ssl_cert(server, user, key, cert, certkey, descr):
    ...
    addCertData = {
     ...
     # Check for errors and assign exit codes accordingly
    certAdded = 10 if check_dns_rebind_error(url, None) else certAdded    # Return exit code 10 if dns rebind error found
    certAdded = 6 if not validate_platform(url, None) else certAdded    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if certAdded == 2:
...
def set_wc_certificate(server, user, key, certName):
    ...
    wccData = {"__csrf_magic" : "", "webguiproto" : PfaVar.wc_protocol, "ssl-certref" : ""}
     # Check for errors and assign exit codes accordingly
    wccCheck = 10 if check_dns_rebind_error(url, None) else wccCheck    # Return exit code 10 if dns rebind error found
    wccCheck = 6 if not validate_platform(url, None) else wccCheck    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if wccCheck == 2:
...
def modify_firewall_alias(server, user, key, aliasName, newValues):
    ...
    if aliasModded == 2:
        ...
        if aliasName in aliasIdData["aliases"]:
            ...
            aliasPostData = {"__csrf_magic" : get_csrf_token(PfaVar.wc_protocol + "://" + server + "/firewall_aliases_edit.php?id=" + aliasIdValue, "GET"), "name" : aliasName, "type" : "host", "tab" : "ip", "id" : aliasIdValue, "save" : "Save"}
            valueToAdd = ""    # Initialize our new alias entry values
            detailToAdd = ""   # Initializes our new alias entry description values
            defaultDetail = "Auto-added by " + user + " on " + PfaVar.local_hostname    # Initializes our default alias entry description value
            ...
            if aliasModded == 2:
                ...
                if check_permissions(postPfAliasData):
                    ...
                    saveChangesPostData = {"__csrf_magic" : get_csrf_token(PfaVar.wc_protocol + "://" + server + "/firewall_aliases.php", "GET"), "apply" : "Apply Changes"}
...
def get_virtual_ips(server, user, key):
    ...
    url = PfaVar.wc_protocol + "://" + server + ":" + str(PfaVar.wc_protocol_port)    # Populate our base URL
     # Check for errors and assign exit codes accordingly
    virtIps["ec"] = 10 if check_dns_rebind_error(url, None) else virtIps["ec"]    # Return exit code 10 if dns rebind error found
    virtIps["ec"] = 6 if not validate_platform(url, None) else virtIps["ec"]    # Check that our URL appears to be pfSense
    # Check if we have not encountered an error that would prevent us from authenticating
    if virtIps["ec"] == 2:
